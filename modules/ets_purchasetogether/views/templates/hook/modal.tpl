            ...
            {foreach from=$products item=product}
                <div class="row">
                  <div class="col-md-5 col-xs-5 product-image-modal">
                    <img class="product-image" src="{$product.product.cover.medium.url}" alt="{$product.product.cover.legend}" title="{$product.product.cover.legend}" itemprop="image">
                  </div>
                  <div class="col-md-7 col-xs-7 product-content-modal">
                    <h6 class="h6 product-name">{$product.product.name}</h6>
                    ...
                    {foreach from=$product.product.attributes item="property_value" key="property"}
                      <span><strong>{$property_value.group}</strong>: {$property_value.name}</span><br>
                    {/foreach}
                    ...
                    {if isset($product.errors) && $product.errors}
                        {foreach from=$product.errors item=error}
                            <p><span class="bg-danger text-white">{$error}</span></p>
                        {/foreach}
                    {*else}
