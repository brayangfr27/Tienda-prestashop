...
<html lang="{$language.iso_code|escape:'html':'UTF-8'}">
  ...
  <body id="{$page.page_name|escape:'html':'UTF-8'}" class="{$page.body_classes|classnames} {if isset($YBC_TC_CLASSES)}{$YBC_TC_CLASSES}{/if}">
                    ...
                    {if $blog_post}
                        ...
                        <div class="post-details">
                            ...
                            {if $display_related_products && $blog_post.products}
                                <div id="ybc-blog-related-products" class="">
                                    <h4 class="h1 products-section-title text-uppercase">
                                        <span>{if count($blog_post.products) > 1}{l s='Related products ' mod='YBC_BLOG_FREE'}
                                        {else}{l s='Related product' mod='YBC_BLOG_FREE'}{/if}</span>
                                    </h4>
                            ...
                            <div class="ybc-blog-related-posts YBC_BLOG_FREE_related_posts_type_{if $blog_related_posts_type}{$blog_related_posts_type|escape:'html':'UTF-8'}{else}default{/if}">
                                <h4 class="h1 products-section-title text-uppercase">
                                    <span>{l s='Related posts' mod='YBC_BLOG_FREE'}</span>
                                </h4>
                                        ...
                                        {foreach from=$blog_post.related_posts item='rpost'}                                            
                                            <li class="ybc-blog-related-posts-list-li thumbnail-container">
                                                ...
                                                <div class="ybc-blog-sidear-post-meta">
                                                    ...
                                                    {/if}
                                                    <span class="post-date">
                                                        {date($date_format,strtotime($rpost.datetime_added))|escape:'html':'UTF-8'}
                                                    </span>
                                                </div>
                                ...
                                {if $allowComments}
                                    ...
                                    {if count($comments)}
                                        ...
                                        <ul class="blog-comments-list">
                                            {foreach from=$comments item='viewcomment'}
                                                
                                                    <li class="blog-comment-line"  itemprop="review" itemscope="" itemtype="http://schema.org/Review">
                                                    <meta itemprop="author" content="{ucfirst($viewcomment.firstname)|escape:'html':'UTF-8'} {ucfirst($viewcomment.lastname)|escape:'html':'UTF-8'}"/>                                                                
                                                    <div class="ybc-blog-detail-comment">
                                                        <h5 class="comment-subject">{$viewcomment.subject|escape:'html':'UTF-8'}</h5>
                                                        {if $viewcomment.firstname || $viewcomment.lastname}<span class="comment-by">{l s='By : ' mod='YBC_BLOG_FREE'}<b>{ucfirst($viewcomment.firstname)|escape:'html':'UTF-8'} {ucfirst($viewcomment.lastname)|escape:'html':'UTF-8'}</b></span>{/if}
                                                        <span class="comment-time"><span>{l s='On' mod='YBC_BLOG_FREE'} </span>{date($date_format,strtotime($viewcomment.datetime_added))|escape:'html':'UTF-8'}</span>
                                                        {if $allow_rating && $viewcomment.rating > 0}
                                                            <div class="comment-rating" itemprop="reviewRating" itemscope itemtype="http://schema.org/Rating">
                                                                <meta itemprop="worstRating" content="0"/>
                                                                <meta itemprop="ratingValue" content="{number_format((float)$viewcomment.rating, 1, '.', '')|escape:'html':'UTF-8'}"/>
                                                                <meta itemprop="bestRating" content="5"/>
                                                                <span>{l s='Rating: ' mod='YBC_BLOG_FREE'}</span>
                                                                <div class="YBC_BLOG_FREE_review">
                                                                    {for $i = 1 to $viewcomment.rating}
                                                                        <div class="star star_on"></div>
                                                                    {/for}
                                                                    {if $viewcomment.rating<5}
                                                                        {for $i = $viewcomment.rating + 1 to 5}
                                                                            <div class="star"></div>
                                                                        {/for}
                                                                    {/if} 
                                                                    <span class="ybc-blog-everage-rating"> {number_format((float)$viewcomment.rating, 1, '.', '')|escape:'html':'UTF-8'}</span>                                     
                                                                </div>
                                                            </div>
                                                        {/if} 
                                                        {if $allow_report_comment}
                                                            {if !($reportedComments && is_array($reportedComments) && in_array($viewcomment.id_comment, $reportedComments))}
                                                                <span class="ybc-block-comment-report comment-report-{$viewcomment.id_comment|intval}" data-rel="{$viewcomment.id_comment|intval}"><i class="fa fa-bolt" aria-hidden="true" title="{l s='Report this comment as abuse' mod='YBC_BLOG_FREE'}"></i></span>
                                                            {/if}
                                                        {/if}
                                                        {if $viewcomment.comment}<p class="comment-content">{$viewcomment.comment|escape:'html':'UTF-8'}</p>{/if}
                                                        
                                                        {if $viewcomment.reply}<p class="comment-reply">
                                                            {if $viewcomment.elastname || $viewcomment.efirstname}
                                                                <span class="ybc-blog-replied-by">
                                                                    {l s='Replied by : ' mod='YBC_BLOG_FREE'}
                                                                    <span class="ybc-blog-replied-by-name">
                                                                        {ucfirst($viewcomment.efirstname)|escape:'html':'UTF-8'} {ucfirst($viewcomment.elastname)|escape:'html':'UTF-8'}
                                                                    </span>
                                                                </span>
                                                            {/if}
                                                            <span class="ybc-blog-reply-content">
                                                                {$viewcomment.reply|escape:'html':'UTF-8'}
                                                            </span></p>
                                                        {/if}
                                                    </div>
                                                    </li>
